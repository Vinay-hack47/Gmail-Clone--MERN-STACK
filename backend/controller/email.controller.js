...
export const createEmail = async (req, res) => {
  try {
    ...
    if (!to || !subject || !message || !deliveryDate) {
    ...
    if (deliveryDateObj > maxDate) {
      return res
        .status(400)
        .json({ msg: "Delivery date cannot exceed 1 year from now" });
    }
    ...
    if (deliveryDateObj > new Date()) {
      ...
      return res
        .status(200)
        .json({ msg: "Email scheduled successfully", success: true, newEmail });
    }
...
export const deleteEmail = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ msg: "Email deleted Successfully", success: true, email });
  } catch (error) {
    console.log(error);
  }
};
...
export const getScheduledEmails = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ msg: "All scheduled emails", success: true, emails });
  } catch (error) {
    console.log(error);
    return res
      .status(400)
...
export const updateScheduledEmail = async (req, res) => {
  try {
    ...
    console.error("Error in updateScheduledEmail:", error);
    return res
      .status(500)
      .json({ msg: "Internal Server Error", error: error.message });
  }
